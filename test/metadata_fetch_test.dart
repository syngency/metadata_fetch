...
void main() {
  ...
  group('Metadata parsers', () {
    ...
    test('JSONLD III', () async {
      var url = 'https://medium.com/@quicky316/install-flutter-sdk-on-windows-without-android-studio-102fdf567ce4';
      var response = await http.get(Uri.parse(url));
      var document = responseToDocument(response);
      // print(response.statusCode);
  ...
  group('extract()', () {
    ...
    test('Youtube Test', () async {
      Metadata data = await (extract('https://www.youtube.com/watch?v=0jz0GAFNNIo') as Metadata);
      expect(data.title, 'Drake - When To Say When & Chicago Freestyle');
      expect(data.image, 'https://i.ytimg.com/vi/0jz0GAFNNIo/maxresdefault.jpg');
    });
...
<html>
    ...
    test("Image url without slash at beginning still results in valid url when falling back to html parser", () {
    ...
    test("MetadataParser.parse(doc) works without a doc.requestUrl (relative URLs are just not resolved)", () {
      final doc = html.parse(htmlPage);
